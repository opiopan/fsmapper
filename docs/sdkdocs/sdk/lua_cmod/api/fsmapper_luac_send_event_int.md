# fsmapper_luac_send_event_int function

The **`fsmapper_luac_send_event_int`** function emits an fsmapper event with an integer value from a Lua C module.
...
```c
void fsmapper_luac_send_event_int(
...
| data | `long long` | An integer value to be used as the event value. |
...
- The integer value passed via `data` is copied internally by fsmapper; the caller may safely reuse the value after the function returns.
