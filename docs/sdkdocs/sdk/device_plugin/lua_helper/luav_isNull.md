...
This function is primarily intended to safely detect the absence of a value when interpreting the `identifier` or `options` parameters passed to the [`FSMDEV_OPEN`](../plugin_abi/FSMDEV_OPEN) callback function.
...
- The returned result must only be used while the [`FSMDEV_OPEN`](../plugin_abi/FSMDEV_OPEN) callback function is executing. Using `LUAVALUE` handles after the callback returns is not allowed.
