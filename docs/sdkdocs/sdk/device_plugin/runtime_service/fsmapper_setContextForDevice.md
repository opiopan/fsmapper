...
This context is typically allocated during the [`FSMDEV_OPEN`](../plugin_abi/FSMDEV_OPEN) callback and allows the plugin to store per-device state that can be retrieved later from other callbacks or runtime service functions.
...
- The device handle becomes invalid after the [`FSMDEV_CLOSE`](../plugin_abi/FSMDEV_CLOSE) callback; the associated context should not be accessed beyond that point.
