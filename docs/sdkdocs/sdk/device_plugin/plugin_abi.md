...
A plugin module is a dynamically loaded library (DLL) that exports a single required entry point function: [`getMapperPluginDeviceOps`](api/getMapperPluginDeviceOps).
...
|---|---|
|[`getMapperPluginDeviceOps`](api/getMapperPluginDeviceOps)|Returns a structure containing the plugin’s callback function pointers and metadata.|
...
|---|---|
|[`FSMDEV_INIT`](api/FSMDEV_INIT)|Initializes plugin-level resources for a Lua script execution.|
|[`FSMDEV_TERM`](api/FSMDEV_TERM)|Releases plugin-level resources when a Lua script terminates.|
|[`FSMDEV_OPEN`](api/FSMDEV_OPEN)|Creates and initializes a device instance.|
|[`FSMDEV_START`](api/FSMDEV_START)|Starts device processing and enables input event generation.|
|[`FSMDEV_CLOSE`](api/FSMDEV_CLOSE)|Closes a device instance and releases device-specific resources.|
|[`FSMDEV_GET_UNIT_NUM`](api/FSMDEV_GET_UNIT_NUM)|Returns the number of device units provided by the device.|
|[`FSMDEV_GET_UNIT_DEF`](api/FSMDEV_GET_UNIT_DEF)|Returns the definition of a device unit.|
|[`FSMDEV_SEND_UNIT_VALUE`](api/FSMDEV_SEND_UNIT_VALUE)|Receives updated values for output device units.|
