...
* Plugins may associate plugin-specific context data with the [`FSMAPPER_HANDLE`](../data_types) by using the fsmapper runtime service function [`fsmapper_setContext`](fsmapper_setContext).
...
* Context data associated with the `FSMAPPER_HANDLE` can later be retrieved by the [`fsmapper_getContext`](fsmapper_getContext) function and used by other callback functions invoked for the same plugin module, allowing shared state to be maintained across callbacks.
